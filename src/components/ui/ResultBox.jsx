...
import { Chip, Card, CardContent, CardActionArea, Button, Link } from '@mui/material';
...
const ResultBox = (props) => {
  ...
  // 'https://5eksdshftf.execute-api.eu-west-1.amazonaws.com/v1/search?online='+online+'&filter=1&query='+searchquery
  const url = props.url;
  // console.log('[ResultBox.js] props.url passed from SearchResultPage: ',url);
  const { data, error, loading } = ApiFunc(url);
  console.log("ResultBox listato data: ",data);
...
{open && (
        ...
        <ListItem id={props.hashid} sx={{
          ...
          {data && <p sx={{ marginBottom: '4rem' }}>{props.boxtype} ({data?.risultati})</p>}
          {/* {!data && <LoadingSpinner />} */}
          </Typography>} 
          />
          ...
          <Link 
          onClick={() => navigate(`/searchfilter?q=${params.get("q")}&fq=${params.get("fq")}&filter=${props.filter}`, {
            state: {
              data_arr: data,
              query: params.get("q"),
          ...
          {!loading && <LoadingSpinner />}
          {error && <p>Errore: {error}</p>}
          {data?.risultati > 0 ? <Button variant="outlined" color="primary" sx={{
            marginRight: '2rem',
            textDecoration: 'none',
          }}
          >
